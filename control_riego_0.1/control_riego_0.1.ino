...
// Variables / constantes de humedad
const byte SENS_HUM[] = {3, 6}; // sensores de humedad
byte cantSensHum = sizeof(SENS_HUM) / sizeof(byte); // cantidad de sensores de humedad
const byte MIN_HUMEDAD = 50; //definida en porcentaje de humedad
const byte MAX_HUMEDAD = 70; //definida en porcentaje de humedad

// Variables / constantes de bombas - relays
const byte BOMBAS_AGUA[] = {1};
byte cantBombasAgua = sizeof(BOMBAS_AGUA) / sizeof(byte); // cantidad de bombas de agua

void setup() {
  // inicializo los sensores de humedad
  for (byte c = 0; c < cantSensHum; c ++) {
    pinMode(SENS_HUM[c], INPUT);
  }

  // inicializo las bombas de agua o relays
  for (byte c = 0; c > cantBombasAgua; c ++)  {
    // faltan inicializar las bombas o relays... es necesario??
  }
}


void loop() {
  byte *flag = necesitaAgua();
   
  // si hay una sola bomba > chequeo que necesiten agua y que no haya ninguna excedida
  if (cantBombasAgua <= 1) {
    bool bombaOn = false;
    bool stopBomba = false;
    
    for (byte c = 0; c < cantSensHum; c ++) {
      if (!stopBomba && *(flag + c) == 2) {
        stopBomba = true;
      } else if (*(flag + c) == 1) {
        bombaOn = true;
      }
    ...
    if (!stopBomba && bombaOn) prenderBomba(0);
  
  // hay más de una bomba y un sensor de humedad por bomba
  } else {
    for (byte c = 0; c < cantSensHum; c ++) {
      if (*(flag + c) == 1) prenderBomba(c);
    } 
  }
}
...
byte * necesitaAgua() {
  ...
  for (byte c = 0; c < cantSensHum; c ++) {
    ...
    humSensor[c] = humedadSensor(c);
    
    if (humSensor[c] <= MIN_HUMEDAD) {
      regar[c] = 1; // necesita agua
    } else if (humSensor[c] < MAX_HUMEDAD) {
      regar[c] = 0; // no necesita agua
    } else  {
      regar[c] = 2; // tiene exceso de agua
    }
  }
...
byte humedadSensor(byte c) {
  int humSensor = map(analogRead(SENS_HUM[c]), 0, 1023, 0, 100);
  return humSensor;
}


// prendo la bomba que me indiquen
void prenderBomba(byte bomba) {
